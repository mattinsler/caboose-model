...
module.exports = class Connection
  ...
  open: (options, callback) ->
    ...
    if options.host?
      server = new mongodb.Server(options.host, options.port ? 27017, options.query)
    else if options.hosts?
      if options.protocol is 'mongos'
        server = new mongodb.Mongos(options.hosts.map((h) ->
          new mongodb.Server(h.host, h.port ? 27017, options.query)
        ))
      else
    ...
    @db.open (err, db) =>
      ...
      return callback?(err, @db) unless options.user? and options.password?
      
      @db.authenticate options.user, options.password, (err) =>
        callback?(err, @db)
